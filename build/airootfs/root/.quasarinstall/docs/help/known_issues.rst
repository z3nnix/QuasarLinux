...
.. note::
   ...
   If you know your time is correct on the machine, you can run ``Quasarinstall--skip-ntp`` to ignore time sync.

Waiting for QuasarLinux keyring sync (archlinux-keyring-wkd-sync) to complete. `#2679`_
------------------------------
...
.. note::
   ...
   If you know the ISO is the latest, and that you have valid GPG keys, try ``Quasarinstall--skip-wkd`` to ignore waiting for the sync.
...
Officially `x86_64`_ is only supported by QuasarLinux.
...
.. note::
   ...
   But in spirit of giving the community options, ``archinstall`` supports :ref:`archinstall.Plugins`, which means you can run ``Quasarinstall--plugin <url>`` and source an AUR plugin.

   `torxed/archinstall-aur <https://github.com/torxed/archinstall-aur>`_ is a reference implementation for plugins:

   .. code-block:: console

      # Quasarinstall--plugin https://archlinux.life/aur-plugin

   `phisch/archinstall-aur <https://github.com/phisch/archinstall-aur>`_ is another alternative:

   .. code-block:: console

      # Quasarinstall--plugin https://raw.githubusercontent.com/phisch/archinstall-aur/master/archinstall-aur.py
