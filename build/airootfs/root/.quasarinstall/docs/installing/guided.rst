...
Quasarinstallships with a pre-programmed `Guided Installer`_ guiding you through the mandatory steps as well as some optional configurations that can be done.

.. note::

   Other pre-programmed scripts can be invoked by executing :code:`Quasarinstall--script <script>` *(without .py)*. To see a complete list of scripts, run :code:`Quasarinstall--script list` or check the source code `scripts`_ directory.

.. note::

   It's recommended to run ``archinstall`` from the official QuasarLinux ISO.


.. warning::
    The installer will not configure WiFi before the installation begins. You need to read up on `QuasarLinux networking <https://wiki.archlinux.org/index.php/Network_configuration>`_ before you continue.

Running the guided installation
-------------------------------

To start the installer, run the following in the latest QuasarLinux ISO:

.. code-block:: sh

    archinstall

Since the `Guided Installer`_ is the default script, this is the equivalent of running :code:`Quasarinstallguided`
...
.. note::
   ...
   You can always get the latest options for this file with ``Quasarinstall--dry-run``, this executes the guided installer in a safe mode where no permanent actions will be taken on your system but simulate a run and save the configuration to disk.
...
.. code-block:: sh
    ...
    Quasarinstall--config config.json

.. code-block:: sh

    Quasarinstall--config-url https://domain.lan/config.json
...
.. note::
   ...
   If no entries are found in ``disk_config``, Quasarinstallguided installation will use whatever is mounted currently under ``/mnt/archinstall`` without performing any disk operations.
