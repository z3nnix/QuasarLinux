...
class Installer:
	...
	def _verify_service_stop(self, offline: bool, skip_ntp: bool, skip_wkd: bool) -> None:
		...
		if not skip_wkd:
			info(tr('Waiting for QuasarLinux keyring sync (archlinux-keyring-wkd-sync) to complete.'))
	...
	def activate_time_synchronization(self) -> None:
		info('Activating systemd-timesyncd for time synchronization using QuasarLinux and ntp.org NTP servers')
	...
	def minimal_installation(
		...
		if ucode := self._get_microcode():
			...
			debug('Quasarinstall will not install any ucode.')
			...
			if not bootloader_removable:
				...
				try:
					SysCommand(
						...
						' --label "QuasarLinux Limine Bootloader"'
	...
	def _add_refind_bootloader(
		...
		for kernel in self.kernels:
			if uki_enabled:
				entry = f'"QuasarLinux ({kernel}) UKI" "{kernel_params}"'
			else:
				if boot_on_root:
				...
				entry = f'"QuasarLinux ({kernel})" "{kernel_params} {initrd_path}"'
	...
	def add_bootloader(
		...
		Quasarinstall supports one of five types:
		* systemd-bootctl
		* grub
		* limine
		* efistub (beta)
	...
	def set_keyboard_language(self, language: str) -> bool:
		...
		if len(language.strip()):
			...
			with Boot(self.target) as session:
				os.system('systemd-run --machine=Quasarinstall--pty localectl set-keymap ""')  # type: ignore[deprecated]
