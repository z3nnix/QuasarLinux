...
install() {
    ...
    sed -i '/Packages from ISO image/d' "$moddir/quasar-installer.sh"
    ...
    # do this...
    sed -i "s:shutdown -r now:sync && reboot -f:" "$moddir/quasar-installer.sh"

    inst "$moddir/quasar-installer.sh" /usr/bin/void-installer
