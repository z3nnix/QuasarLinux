...
usage() {
	...
	Wrapper script around quasar-live.sh for several standard flavors of live images.
	...
	Other options can be passed directly to quasar-live.sh by specifying them after the --.
	See quasar-live.sh -h for more details.
	EOH
}
...
include_installer() {
    if [ -x quasar-installer.sh ]; then
        MKLIVE_VERSION="$(PROGNAME='' version)"
        installer=$(mktemp)
        sed "s/@@MKLIVE_VERSION@@/${MKLIVE_VERSION}/" quasar-installer.sh > "$installer"
        install -Dm755 "$installer" "$INCLUDEDIR"/usr/bin/void-installer
        rm "$installer"
    else
        echo quasar-installer.sh not found >&2
        exit 1
    fi
}
...
build_variant() {
    variant="$1"
    shift
    IMG=quasar-live-${ARCH}-${DATE}-${variant}.iso
    ...
    A11Y_PKGS="espeakup quasar-live-audio brltty"
...
EOF
    ...
    ./quasar-live.sh -a "$TARGET_ARCH" -o "$IMG" -p "$PKGS" -S "$SERVICES" -I "$INCLUDEDIR" \
        ${KERNEL_PKG:+-v $KERNEL_PKG} ${REPO} "$@"

	cleanup
}

if [ ! -x quasar-live.sh ]; then
    echo quasar-live.sh not found >&2
    exit 1
fi
